...
p.add_argument("--clip", type=float, default=0.02,
               help="integration step used by C9 and C11; match the campaign")
...
for b in a.b:
    ...
    if "C9"  in want: add(K.c9_rest_points(net, b, clip=a.clip))
    if "C10" in want: add(K.c10_certificate(b, a.cert_sinks, kind="knn", knn=a.cert_knn))
    if "C11" in want: add(K.c11_numerics(net, b, clip=a.clip))
